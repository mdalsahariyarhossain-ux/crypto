...
const MAIN_TOOLS = [
  ...
  { path: "/benchmark",             icon: "⚖️", tag: "11 Steps",    tagClass: "text-amber-400 bg-amber-500/10 border-amber-500/20", title: "RSA & ECC Comparison Wizard",     cta: "Start →",accent: "hover:border-amber-400/40" },
...
function QuickStartCard() {
  ...
  function startWizard() {
    navigate("/benchmark", {
      state: { keyConfig, inputText, inputBuffer, uploadedFile: uploadedFile ? { name: uploadedFile.name, size: uploadedFile.size } : null }
    });
  }

  const canStart = true; // always allow — wizard handles validation
    ...
    <div className="space-y-6">
      ...
      <div className="relative text-center py-8 px-4 rounded-2xl border border-slate-700/60 bg-gradient-to-br from-slate-900/90 to-slate-800/60 overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="flex justify-center gap-8 flex-wrap">
            {[["RSA & ECC","Algorithms"],["11 Steps","Wizard"],["AI","Advisor"],["100%","Client-side"]].map(([v,l])=>(
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
          {TOOLKIT.map(c => <ToolCard key={c.title} c={c} />)}
        </div>
      </div>
