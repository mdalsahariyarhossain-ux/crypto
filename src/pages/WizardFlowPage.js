...
export default function WizardFlowPage() {
  ...
  const [inputBuffer] = useState(fromHome?.inputBuffer || null); // ArrayBuffer
  ...
  // Step 9: Ask AI Advisor for Recommendations
  const fetchAiReport = async () => {
    ...
    try {
      ...
      if (maxCompletedStep < 9) setMaxCompletedStep(9);
    } catch (e) {
      console.error(e);
  ...
  // Config summary bar — shown on every step so the active RSA/ECC config
  // and input are always visible, with a quick way back to the home page
  // to change them (that's where they're actually chosen now).
  const ConfigBar = () => (
    <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-gradient-to-r from-slate-900 via-slate-900/95 to-slate-800/70 p-4 sm:p-5 mb-6 flex flex-wrap items-center justify-between gap-4 animate-fadeIn">
      ...
      <div className="relative flex items-center gap-3 sm:gap-4 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-[9px] font-bold tracking-widest text-slate-500 uppercase">Active configuration</p>
          <p className="text-[13px] font-bold text-white truncate">
            RSA-{rsaBits} <span className="text-slate-600">↔</span> ECC-{eccCurve}
            <span className="mx-2 text-slate-700">•</span>
            <span className="text-slate-300 font-medium">
              {uploadedFile ? `📂 ${uploadedFile.name}` : `✏️ ${inputText.length} chars of text`}
            </span>
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="lg:col-span-3 bg-gradient-to-br from-slate-900/80 to-slate-950 border border-slate-800 rounded-3xl p-6 shadow-2xl relative min-h-[500px] flex flex-col justify-between overflow-hidden">
          ...
          <div className="relative z-10 flex-1">
            ...
            {activeStep === 1 && (
              <div className="space-y-6 animate-fadeIn">
                <p className="text-sm text-slate-400">
                  Generate key pairs locally in your browser for the RSA-{rsaBits} ↔ ECC-{eccCurve} configuration you chose on the home page. This produces encryption key pairs (RSA-OAEP, ECDH) and signing key pairs (RSA-PSS, ECDSA).
                </p>
            ...
            {/* STEP 2: Encryption of File */}
            {activeStep === 2 && (
              <div className="space-y-6 animate-fadeIn">
                <p className="text-sm text-slate-400">
                  Encrypt your input using the generated RSA and ECC key pairs. Download buttons will be available for each encrypted output.
                </p>
            ...
            {/* STEP 4: Encryption Benchmark (30 runs) */}
            {activeStep === 4 && (
              <div className="space-y-6 animate-fadeIn">
                <p className="text-sm text-slate-400">
            ...
            {/* STEP 5: Performance Graph Chart of Both RSA and ECC for Encryption */}
            {activeStep === 5 && (
              <div className="space-y-6 animate-fadeIn">
                <p className="text-sm text-slate-400">
            ...
            {/* STEP 7: Decryption Time Bar Graph (30 Times) */}
            {activeStep === 7 && (
              <div className="space-y-6 animate-fadeIn">
                <p className="text-sm text-slate-400">
            ...
            {/* STEP 8: Performance Graph Chart of Both RSA and ECC for Decryption */}
            {activeStep === 8 && (
              <div className="space-y-6 animate-fadeIn">
                <p className="text-sm text-slate-400">
                ...
                {decBenchRaw.length > 0 ? (
                  ...
                  <div className="bg-slate-900 border border-slate-850 rounded-2xl p-12 text-center text-slate-500">
                    Please run the benchmark in Step 7 first to generate data.
                  </div>
                )}
              </div>
            )}

            {/* STEP 9: Overall Performance Summary */}
            {activeStep === 9 && (
              <div className="space-y-6 animate-fadeIn">
                <p className="text-sm text-slate-400">
